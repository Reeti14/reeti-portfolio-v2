...
export default function ProjectCard({ project }: ProjectProps) {
  ...
  return (
    <motion.div
      ref={cardRef}
      className="group relative flex flex-col h-full glass rounded-xl overflow-hidden shadow-sm transition-all glow-border cursor-pointer card-3d"
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-grow relative z-10 card-3d-inner" style={{ transformStyle: "preserve-3d" }}>
        <div className="flex justify-between items-start mb-4" style={{ transform: "translateZ(30px)" }}>
          <h3 className="text-xl font-bold text-sage-dark group-hover:text-terracotta transition-colors duration-300">{project.title}</h3>
          
          <div className="flex gap-2">
            {project.github && project.github !== "#" && (
              <a href={project.github} target="_blank" rel="noopener noreferrer" className="p-2 text-sage hover:text-cream hover:bg-sage rounded-lg transition-all btn-3d" aria-label="GitHub Repository">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"></path></svg>
              </a>
            )}
            {project.link && project.link !== "#" && (
              <a href={project.link} target="_blank" rel="noopener noreferrer" className="p-2 text-sage hover:text-cream hover:bg-terracotta rounded-lg transition-all btn-3d" aria-label="Live Demo">
